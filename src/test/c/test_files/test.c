...
ct_assert_report_t* ct_init_assert_report(_Bool is_mandatory, char* asserted_text, char* file, unsigned int line);
...
typedef _Bool (*condition_section)(crashc_model * model, Section*);
...
_Bool runOnceAndCheckAccessToSection(crashc_model * model, Section* section, condition_section cs, BeforeStartingSectionCallBack callback, const tag_ht* restrict runOnlyIfTags, const tag_ht* restrict excludeIfTags);
_Bool runOnceAndDoWorkAtEnd(crashc_model * model, Section* section, Section** pointerToSetAsParent, AfterExecutedSectionCallBack callback, AfterExecutedSectionCallBack accessGrantedCallback, AfterExecutedSectionCallBack accessDeniedCallback);
...
_Bool getAccess_When(crashc_model * model, Section * section);
_Bool getAlwaysTrue(crashc_model * model, Section* section);
_Bool getAccessSequentially(Section* section);
...
int main(const int argc, const char** args) { cc_model = setupDefaultMainModel(); parseCommandLineArguments(argc, args, ' ', (&cc_model)->runOnlyIfTags, (&cc_model)->excludeTags); registerSignalHandlerForSignals();
void suite_execution_flow(); update_test_array((&cc_model), suite_execution_flow);
for (int i = 0; i < (&cc_model)->suites_array_index; i++) { (&cc_model)->tests_array[i](); } (&cc_model)->report_producer_implementation->report_producer(&cc_model); if ((&cc_model)->ct_teardown != ((void *)0)) { (&cc_model)->ct_teardown(); } tearDownDefaultModel(cc_model); }
void suite_execution_flow() {
 clearTestChecker();
 ((&cc_model))->currentSection = getSectionOrCreateIfNotExist(((&cc_model)->rootSection), ST_TESTCASE, "passed assertion simple", ""); ((&cc_model))->currentSection->loopId += 1; (&cc_model)->jump_source_testcase = (&cc_model)->currentSection; _Bool signalDetected14 = 0; int jump_code14 = __sigsetjmp ((&cc_model)->jump_point, 1); if (jump_code14 == 1) { markSectionAsSignalDetected((&cc_model)->currentSection); signalDetected14 = 1; ct_reset_section_after_jump((&cc_model), (&cc_model)->currentSection, (&cc_model)->jump_source_testcase); } else if (jump_code14 == 2) { ct_reset_section_after_jump((&cc_model), (&cc_model)->currentSection, (&cc_model)->jump_source_testcase); } for ( ; !signalDetected14 && sectionStillNeedsExecution((&cc_model)->currentSection) ; ) for ( ((&cc_model))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd(((&cc_model)), ((&cc_model))->currentSection, &(((&cc_model))->currentSection), doWorkAtEndCallbackResetContainer, callbackExitAccessGrantedTestcase, doWorkAtEndCallbackDoNothing ) ; ((&cc_model))->currentSection->loop1 = 0 ) for ( ((&cc_model))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection(((&cc_model)), ((&cc_model))->currentSection, getAlwaysTrue, callbackEnteringTestcase, ((&cc_model))->runOnlyIfTags, ((&cc_model))->excludeTags) ; ((&cc_model))->currentSection->loop2 = 0, markSectionAsExecuted(((&cc_model))->currentSection) ) {
  addCharacter('a');
  (((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_WHEN, "", ""); (((&cc_model)))->currentSection->loopId += 1; for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 ) for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAccess_When, callbackEnteringWhen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   addCharacter('b');
   addTailInList((&cc_model)->currentSnapshot->assertion_reports, ct_init_assert_report(1, "1", "test_issue0015.c", 18)); if ((1) != 1) { ct_general_assert_failed((&cc_model)); } else { ct_assert_do_nothing((&cc_model)); };
   addCharacter('d');
  }
  (((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_WHEN, "", ""); (((&cc_model)))->currentSection->loopId += 1; for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 ) for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAccess_When, callbackEnteringWhen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   addCharacter('c');
   addTailInList((&cc_model)->currentSnapshot->assertion_reports, ct_init_assert_report(1, "1", "test_issue0015.c", 23)); if ((1) != 1) { ct_general_assert_failed((&cc_model)); } else { ct_assert_do_nothing((&cc_model)); };
   addCharacter('e');
  }
  addCharacter(' ');
 }
 assertTestCheckerAndReset("abd ace ");
 ((&cc_model))->currentSection = getSectionOrCreateIfNotExist(((&cc_model)->rootSection), ST_TESTCASE, "failed assertion simple", ""); ((&cc_model))->currentSection->loopId += 1; (&cc_model)->jump_source_testcase = (&cc_model)->currentSection; _Bool signalDetected31 = 0; int jump_code31 = __sigsetjmp ((&cc_model)->jump_point, 1); if (jump_code31 == 1) { markSectionAsSignalDetected((&cc_model)->currentSection); signalDetected31 = 1; ct_reset_section_after_jump((&cc_model), (&cc_model)->currentSection, (&cc_model)->jump_source_testcase); } else if (jump_code31 == 2) { ct_reset_section_after_jump((&cc_model), (&cc_model)->currentSection, (&cc_model)->jump_source_testcase); } for ( ; !signalDetected31 && sectionStillNeedsExecution((&cc_model)->currentSection) ; ) for ( ((&cc_model))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd(((&cc_model)), ((&cc_model))->currentSection, &(((&cc_model))->currentSection), doWorkAtEndCallbackResetContainer, callbackExitAccessGrantedTestcase, doWorkAtEndCallbackDoNothing ) ; ((&cc_model))->currentSection->loop1 = 0 ) for ( ((&cc_model))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection(((&cc_model)), ((&cc_model))->currentSection, getAlwaysTrue, callbackEnteringTestcase, ((&cc_model))->runOnlyIfTags, ((&cc_model))->excludeTags) ; ((&cc_model))->currentSection->loop2 = 0, markSectionAsExecuted(((&cc_model))->currentSection) ) {
  addCharacter('a');
  (((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_WHEN, "W1", ""); (((&cc_model)))->currentSection->loopId += 1; for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 ) for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAccess_When, callbackEnteringWhen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   addCharacter('b');
  }
  (((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_WHEN, "W2", ""); (((&cc_model)))->currentSection->loopId += 1; for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 ) for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAccess_When, callbackEnteringWhen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   addCharacter('c');
   addTailInList((&cc_model)->currentSnapshot->assertion_reports, ct_init_assert_report(1, "0", "test_issue0015.c", 38)); if ((0) != 1) { ct_general_assert_failed((&cc_model)); } else { ct_assert_do_nothing((&cc_model)); };
   addCharacter('x');
  }
  (((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_WHEN, "W3", ""); (((&cc_model)))->currentSection->loopId += 1; for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 ) for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAccess_When, callbackEnteringWhen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   addCharacter('d');
  }
  	(((&cc_model)))->currentSection = getSectionOrCreateIfNotExist(((&cc_model))->currentSection, ST_THEN, "T", ""); 
  	(((&cc_model)))->currentSection->loopId += 1; 
  	for ( (((&cc_model)))->currentSection->loop1 = 1 ; runOnceAndDoWorkAtEnd((((&cc_model))), (((&cc_model)))->currentSection, &((((&cc_model)))->currentSection), doWorkAtEndCallbackGoToParentAndThenToNextSibling, doWorkAtEndCallbackChildrenNumberComputed, doWorkAtEndCallbackDoNothing ) ; (((&cc_model)))->currentSection->loop1 = 0 )
  	 	for ( (((&cc_model)))->currentSection->loop2 = 1 ; runOnceAndCheckAccessToSection((((&cc_model))), (((&cc_model)))->currentSection, getAlwaysTrue, callbackEnteringThen, (((&cc_model)))->runOnlyIfTags, (((&cc_model)))->excludeTags) ; (((&cc_model)))->currentSection->loop2 = 0, markSectionAsExecuted((((&cc_model)))->currentSection) ) {
   	addCharacter('e');
  }
  addCharacter(' ');
 }
 assertTestCheckerAndReset("abe ac ade ");
}
